...
// GetNodeCerts get node certs
func (k *Krypton) GetNodeCerts(ctx context.Context, podname, nodename string) (string, string, string) {
	ca, err := k.etcd.Get(ctx, fmt.Sprintf(nodeCaKey, podname, nodename), nil)
	if err != nil {
		return "", "", ""
	}
	certs, err := k.etcd.Get(ctx, fmt.Sprintf(nodeCertKey, podname, nodename), nil)
	if err != nil {
		return "", "", ""
	}
	key, err := k.etcd.Get(ctx, fmt.Sprintf(nodeKeyKey, podname, nodename), nil)
	if err != nil {
		return "", "", ""
	}
	return ca.Node.Value, certs.Node.Value, key.Node.Value
}
