...
func (k *Krypton) WatchDeployStatus(ctx context.Context, appname, entrypoint, nodename string) chan *types.DeployStatus {
	if appname == "" {
		entrypoint = ""
	}
	if entrypoint == "" {
	...
	go func() {
		defer close(ch)
		watcher := k.etcd.Watcher(key, &etcdclient.WatcherOptions{Recursive: true})
		for {
